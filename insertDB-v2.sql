...
/*
select (S_Fname ||' '|| S_Lname) as full_name, Salary 
from SubContractor sb where sb.Salary > '3000000';
*/
...
select (B_Lname ||' '|| B_Fname) as Full_Name , FinishDate
from Builder bl join House hs 
on bl.BID = hs.BID where FinishDate > now() ORDER BY Full_Name;
...
SELECT /*BID,*/ B_Lname, /*SID,*/ S_Lname, Salary FROM SubContractor sc NATURAL JOIN 
...
SELECT B_Lname, B_Fname, H_Name FROM Builder bd JOIN House h on bd.BID = h.BID 
...
SELECT N_Name, City FROM Neighborhood n JOIN House h ON n.NID = h.NID 
